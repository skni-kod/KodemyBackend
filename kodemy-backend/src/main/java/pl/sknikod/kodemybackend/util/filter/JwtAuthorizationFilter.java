...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            if (jwt != null && jwtUtil.isTokenValid(jwt)) {
                ...
                var authentication = new UsernamePasswordAuthenticationToken(jwtUserDetails, null, jwtUserDetails.getAuthorities());
    ...
    private SecurityConfig.JwtUserDetails mapToUserDetails(JwtUtil.Output.Deserialize deserialize) {
        var deserializedAuthorities = deserialize.getAuthorities().stream().map(SimpleGrantedAuthority::new).collect(Collectors.toSet());
