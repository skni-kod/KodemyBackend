...
class UserStoreHandlerTest extends BaseTest {
    @Mock
    ...
    @BeforeEach
    void setUp() {
        userStoreHandler = new UserStoreHandler(userRepository, roleRepository, roleProperties);
    }
    ...
    void save_shouldSucceed() {
        ...
        // when
        Optional<User> result = userStoreHandler.save(OAuth2Factory.oAuth2GithubUser());
        // then
    ...
    void findById_shouldSucceed() {
        ...
        // when
        Try<User> result = userStoreHandler.findById(UserFactory.userUser.getId());
        // then
        assertTrue(result.isSuccess());
        assertEquals(UserFactory.userUser.getUsername(), result.get().getUsername());
    }
    ...
    void findByUsernameOrEmailOrRole_shouldSucceed() {
        ...
        // when
        Page<User> result = userStoreHandler.findByUsernameOrEmailOrRole(any(), any(), any(), any());
        // then
